...
- **Ghostty を更新したとき**（ghostty モード）: 何もしなくてよい。次の起動時にスタブが検知して自動で作り直す（数秒かかる）
- **このリポジトリを更新したとき**: `git pull && ./build-herdr-app.sh`。Dock のピン留めはそのまま生きる

### アンインストール

```sh
rm -rf /Applications/Herdr.app
defaults delete io.github.ebi-oishii.herdr-shortcut 2>/dev/null; killall Dock
```
...
- **クリックしてもウィンドウが出ない（プロセスだけ居る）**: もう一度 Dock アイコンをクリックすると開く。ビルド直後の初回起動でまれに起きる
- **Dock のアイコンがおかしい／変わらない**: `defaults delete io.github.ebi-oishii.herdr-shortcut CustomGhosttyIcon2; killall Dock`。それでも直らなければ本家 Ghostty 側のドメイン（`com.mitchellh.ghostty`）にも同じキーが残っていないか確認
- **同じ画面が 2 つのウィンドウに映る／画面サイズが小さくなる**: 同一セッションに複数クライアントが同時アタッチしている（herdr の仕様、tmux と同じミラー動作）。別セッションに分けるか、片方を閉じる
- **軽量モードでクリックしても反応しない**: 初回の Automation 許可ダイアログを拒否した可能性。システム設定 → プライバシーとセキュリティ → オートメーション で Herdr に許可を出す
...
| ファイル | 役割 |
|---|---|
| `build-herdr-app.sh` | 本体。`/Applications/Herdr.app` を生成する |
| `herdr-launcher.c` | ghostty モードの起動スタブ（自動リビルド判定 → 同梱 ghostty を exec） |
| `herdr-session.sh.template` | セッションループ（デタッチ後のメニュー・タイトル表示） |
| `herdr.conf.template` | Herdr.app 専用の Ghostty 設定 |
| `generate-icons.sh` / `squircle-icon.swift` | 公式ロゴの取得とアイコン生成 |
| `make_icon.swift` | （おまけ）自作羊アイコンのジェネレータ |
